...
/**
 * @brief Выводит таблицу с данными о сотрудниках в консоль.
 *
 * Данная функция принимает вектор структур `worker`, который содержит информацию о сотрудниках,
 * их отработанном времени по месяцам, и выводит эти данные в консоль в виде таблицы.
 * Таблица состоит из трех столбцов: "Имя сотрудника", "Месяц" и "Отработанное время".
 * Время работы сотрудников выводится в формате "дни:часы:минуты".
 * Месяцы отображаются на русском языке с использованием заранее подготовленного словаря.
 *
 * @param data Вектор структур `worker`, содержащий:
 *             - имя сотрудника (name);
 *             - список месяцев с отработанным временем (work_time), где каждый месяц представлен числом,
 *               а время работы — количеством минут.
 */
void printTable(const vector<worker>& data) {
    // Словарь для отображения номеров месяцев в их русские названия
    map<int, string> month_names = {
        {1, "Январь"}, {2, "Февраль"}, {3, "Март"}, {4, "Апрель"},
        {5, "Май"}, {6, "Июнь"}, {7, "Июль"}, {8, "Август"},
        {9, "Сентябрь"}, {10, "Октябрь"}, {11, "Ноябрь"}, {12, "Декабрь"}
